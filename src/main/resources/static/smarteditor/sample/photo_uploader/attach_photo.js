...
//    	sUploadURL= '/board/multiFileUpload'; 	//upload URL
	...
	function callAjaxForHTML5(tempFile, sUploadURL) {
            ...
            formData.append("files", tempFile);  // 파일을 FormData에 추가
                ...
                if (!allowedTypes.includes(tempFile.type)) {
                    ...
                    return;  // 이미지가 아닌 파일은 업로드하지 않음
                }

            // FormData에 추가된 값 출력 (디버깅용)
            console.log([...formData]);

            var oAjax = new XMLHttpRequest();
            oAjax.open('POST', sUploadURL, true);

            oAjax.onload = function() {
                if (oAjax.status === 200) {
                    console.log('Upload successful');
                    makeArrayFromString(oAjax.responseText);
                } else {
            ...
            oAjax.send(formData);  // 파일을 업로드하기 위한 요청을 보냄
        }
   ...
   /* function makeArrayFromString(sResString){
        ...
        console.log(aResultleng);
        console.log(aResult[aResultleng]);
        console.log(nImageFileCount);
        console.log(nImageInfoCnt);
    	...
    	aResult[aResultleng] = htTemp;setPhotoToEditor;
    ...
    function makeArrayFromString(sResString){
        ...
        console.log(aResultleng);
        console.log(sResString.indexOf("sFileURL"));
        console.log(nImageFileCount);
        console.log(nImageInfoCnt);

        try {
            if (!sResString) {
                console.error("sResString is empty or null");
                return;
            }

            // JSON 파싱
            var jsonResponse = JSON.parse(sResString);
            ...
            if (Array.isArray(jsonResponse)) {
                jsonResponse.forEach(function (item) {
                    var htTemp = {};
                    htTemp["uuid"] = item.uuid;
                    htTemp["saveDir"] = item.saveDir;
                    htTemp["fileName"] = item.fileName;
                    htTemp["fileType"] = item.fileType;
                    htTemp["bno"] = item.bno;
                    htTemp["fileSize"] = item.fileSize;
                    htTemp["fileUrl"] = item.fileUrl
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/board/singleIamgeUpload',	//샘플 URL입니다.
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
 	    console.log(oFileInfo)
		    // oFileInfo 배열이 비어있지 않고, 각 파일에 fileUrl이 유효한지 확인
             if (oFileInfo && oFileInfo.length > 0) {
                 let imgTags = '';  // 이미지 태그를 하나의 문자열로 모을 변수

                 // oFileInfo 배열을 순회하면서 각 파일에 대해 <img> 태그를 생성
                 for (let i = 0; i < oFileInfo.length; i++) {
                     if (oFileInfo[i] && oFileInfo[i].fileUrl) {
                         // 각 파일에 대해 <img> 태그를 생성
                         let imgTag = `<img src="${oFileInfo[i].fileUrl}" alt="${oFileInfo[i].fileName}" />`;
                         imgTags += imgTag;  // 생성된 <img> 태그를 문자열에 추가
                     } else {
                         console.error(`유효한 이미지 URL이 없습니다. 파일 ${i + 1}`);
                     }
                 }

                 // 스마트 에디터 플러그인을 통해서 이미지 태그를 삽입하는 방법
                 if (imgTags && !!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
                     opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', [imgTags]);
                 }
             } else {
                 console.error("유효한 이미지 URL이 없습니다.");
             }
	}
